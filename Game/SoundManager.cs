...
namespace GameProject
{
    public class SoundEffectPlaying
    {
        ...
        public SoundEffectPlaying(string path, bool looping, int type, float volume, float pitch = 0f, float pan = 0f)
        {
            var sfx = Assets.Content.Load<SoundEffect>(path);
    ...
    public class SoundManager
    {
        ...
        public int PlaySound(string assetName, int type, bool loop = false, bool allowDuplicate = false)
        {
            ...
            {
                for (var i = 0; i < _soundEffectsPlaying.Count; i++)
                    if (_soundEffectsPlaying.ElementAt(i).Value.Path == assetName)
                        return -1;
            }
